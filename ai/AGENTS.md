...
- **The ArtSoft Consult-specific scraping logic is in `scraper/index.js`** (`fetchJobsHtml`, `parseHtmlJobs`, `parseInternshipPage`). Uses HTML scraping with cheerio on `div.single-job-container`. The output shape (`mapToJobModel`, `transformJobsForSOLR`) is inherited from the template and must NOT change — that's what keeps SOLR uniform across derived scrapers.
...
```bash
gh run view <RUN_ID> --repo sebiboga/artsoft-consult-srl-nodejs-scraper --json status -q .status
```
...
**On every session:**
1. Check open GitHub issues: `gh issue list --repo sebiboga/artsoft-consult-srl-nodejs-scraper --state open`

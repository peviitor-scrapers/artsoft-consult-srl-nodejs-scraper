...
| File | Description |
|------|-------------|
| `scraper/index.js` | Main scraper - full workflow: validate company → scrape → transform → upsert → generate docs/jobs.md. ArtSoft Consult specific: `fetchJobsHtml` + `parseHtmlJobs` (cheerio) + `parseInternshipPage` |
...
| `scraper/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), validateByBrowser(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by `validate-jobs.js`, `tests/validate-artsoft-consult-jobs.js`, and the deep-validate workflow. |
...
| `tests/validate-artsoft-consult-jobs.js` | **ArtSoft-specific validator (used by CI).** Modes: `--head` (default), `--content`, `--browser` (Playwright). Called nightly by `automation-testing.yml` and manually via `job-deep-validate.yml`. Supports `--dry-run` and `--delete`. |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real ArtSoft Consult website, ANAF, and Peviitor API |
...
| `tests/consistency/version.test.js` | Verifies version consistency across package.json |
...
| `ai/AI-DERIVATION-GUIDE.md` | Comprehensive playbook for AI agents deriving a new scraper from the template |
...
| `ai/ROBOTS.md` | robots.txt analysis and scraping policy for ArtSoft Consult |
...
| `.github/workflows/job-recovery-from-disaster.yml` | Manual recovery: deletes stale jobs and re-scrapes |
...
| `scraper/config/company.json` | **Source of truth.** Committed. Refreshed when company data changes. |
...
- Full workflow: validate company (ANAF+CUIScan+CUIFirma+Peviitor) → scrape ArtSoft Consult HTML → transform → upsert → generate docs/jobs.md
